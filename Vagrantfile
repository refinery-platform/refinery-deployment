...
<<SCRIPT
  ...
  cd /vagrant/puppet && \
  librarian-puppet config tmp /tmp/puppet --global && \
...
#    puppet.module_path = "puppet/modules"  # requires modules dir to exist when this file is parsed
    puppet.options = "--modulepath /vagrant/puppet/modules"
  end
end
